  ...
  return (
    <div className=" dark:bg-gray-800 dark:text-white w-full h-auto overflow-y-scroll scrollbar-hide flex flex-col items-center gap-6 ">
      {EduvidContent.map((item) => (
        <div
          key={item.id}
          className="w-[440px] h-[620px] bg-[#1f002b] rounded-3xl  text-white font-sans relative shadow-lg"
        >
          {/* Background */}
    ...
    {/* Sidebar Icons */}
<div className="absolute right-4 top-[65%] transform -translate-y-1/2 flex flex-col items-center space-y-6 p-4 rounded-2xl shadow-lg ">
  {/* Like */}
  <div className="flex flex-col items-center gap-2 cursor-pointer">
    <img src={thumbLike} alt="Like" className="w-[32px] h-[32px] bg-black/40 p-1 rounded-full" />
    <p className="text-base font-medium text-white">{item.likes}</p>
  </div>

  {/* Comment */}
  <div className="flex flex-col items-center gap-2 cursor-pointer">
    <img src={comment} alt="Comment" className="w-[32px] h-[32px] bg-black/40 p-1  rounded-full" />
    <p className="text-base font-medium text-white">{item.comments}</p>
  </div>

  {/* Share */}
  <div className="flex flex-col items-center gap-2 cursor-pointer">
    <img src={share} alt="Share" className="w-[32px] h-[32px] bg-black/40 p-1 rounded-full" />
    ...
    <img src={elipe} alt="." className=" opacity-80" />
            ...
            <div className="mt-auto">
              <div className="flex items-center gap-2 mb-2">
                ...
                <button className="ml-10 border border-white px-2 py-0.5 rounded-md text-xs">
                  Follow
                </button>
              </div>
